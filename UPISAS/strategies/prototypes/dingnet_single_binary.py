...
class DingNetEnv(gym.Env):
    ...
    def __init__(self, render_mode: Optional[str]=None, num_gateways=DEFAULT_NUM_GATEWAYS,
        ...
        self.action_space = spaces.Discrete(2)  # 0: Decrease power, 1: Increase power
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        if options is not None:
            ...
            map_refresh = options.get('map_refresh') if 'map_refresh' in options else True
    ...
    def _observe(self):
        ...
        return np.array([signal_strength, min_distance, self.mote['power'], self.mote['position'][0], self.mote['position'][1], path_loss, shadow_fading], dtype=np.float32)
    ...
    def _normalize(self, observations):
        ...
        for i in range(len(observations)):
            observations[i] = 2 * ((observations[i] - self.observation_space[i].low) / (self.observation_space[i].high - self.observation_space[i].low))
        return observations
